...
public class MyCompactTreeLayout extends mxGraphLayout
{
	...
	/**
	 * Specifies the maximum node size. Is necessary to keep all nodes on different branches on the same x-level.
	 */
	 ...
	 * @param graph
	 */
	public MyCompactTreeLayout(mxGraph graph)
	{
		this(graph, true);
	}
	 ...
	 * @param horizontal
	 */
	public MyCompactTreeLayout(mxGraph graph, boolean horizontal)
	{
	 ...
	 * @param invert
	 */
	public MyCompactTreeLayout(mxGraph graph, boolean horizontal, boolean invert)
	{
		super(graph);
...
/*		node.contour.upperTail = createLine(node.height + dist, 0, null);
		...
		node.contour.lowerHead = createLine(maxNodeSize + dist, 0,
				node.contour.lowerTail);
	}
	...
	protected double join(TreeNode node)
	{
		...
		double h = /*child.width*/ maxNodeSize + dist;
		double sum = h;
		child = child.next;
		...
		while (child != null)
		{
			...
			child.offsetY = d + h;
			child.offsetX = 0;
			h = /*child.width*/ maxNodeSize + dist;
			sum += d + h;
			child = child.next;
		}
	...
	protected void processNodeOutgoing(TreeNode node)
	{
		...
		mxGeometry geom =((mxCell)parentCell).getGeometry();
